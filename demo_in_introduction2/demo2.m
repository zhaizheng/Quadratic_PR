...
function step1()
    [X1, X2, X] = generate_sphere(0.08, 2, 300, 300);
    sigma = 0.4;
...
function result = step2()
    [X1, X2, ~] = generate_sphere(0.04, 2, 300, 300);
    sigma = 0.1:0.1:0.9;
    result = zeros(2, 9);
    for i = 1:9
        ...
        result(4,i) = max_distance(data6, bsxfun(@rdivide, data6, sqrt(sum(data6.^2,1))));
        result(5,i) = max_distance(data6n, bsxfun(@rdivide, data6n, sqrt(sum(data6n.^2,1))));
        result(6,i) = max_distance(data6nn, bsxfun(@rdivide, data6nn, sqrt(sum(data6nn.^2,1))));
    end
%     plot(result(1,:),'-*')
%     hold on
%     plot(result(2,:),'-o')
    subplot(1,2,1)
    semilogy(sigma,result(1,:),'-*'); hold on; semilogy(sigma, result(2,:),'--o'); hold on; semilogy(sigma, result(3,:),'--s');
    legend('KDE','LKDE')
    xlabel('h')
    ylabel('margin')
    ...
    subplot(1,2,2)
    semilogy(sigma,result(4,:),'-*'); hold on; semilogy(sigma, result(5,:),'--o'); hold on; semilogy(sigma, result(6,:),'--s')
    legend('KDE','LKDE')
    xlabel('h')
    ylabel('margin')
    title('Hausdorff')
...
function [steps, data_move] = algorithm(data, data_move, sigma, algo)
    ...
    for i = 1:n
        for k = 1:max_iter
            ...
            elseif algo == 8
                direction = xia(data_move(:,i), data, 0.4, 2, 1, step);
            end

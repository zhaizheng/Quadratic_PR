...
for i = 1:length(Neig)
    ...
    %    img = floor(256*rand(d,1));
        ...
        for k = 1:4
            output{k,rep,i}= run_algorithm(img, data, d, step, k, Neig(i));
            error(k,rep,i) = norm(lD(:,rep)-output{k, rep, i},'fro');
        end
...
for neig = 1:4
    %subplot(1,6,neig+1)
    subplot('Position',[(0.03+0.15)*neig-0.04 0.1 0.15,0.85])
    imag = [];
    for rep = 1:6
        row = [];
        for alg = 1:4
...
legend('{\it l}-SCRE','SCRE', 'MFIT-ii','MFIT-i');
set(gca,'FontSize',12);
%%
function obj = run_algorithm(obj, data, d, step, alg, k)
    ...
    for rep = 1:10000
        switch alg
            case 1
                direction = Hc77(obj, sigma, data, d, step);
            case 2
                direction = Hc1(obj, sigma, data, d, step);
            case 3
